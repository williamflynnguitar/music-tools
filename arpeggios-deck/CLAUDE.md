...
4. **VDA** (Ex. 4): scale position drawn as a hollow outline; per chord, the fingering (any root string) with the most notes inside the window, tie-broken by notes that coincide with the scale shape. Notes outside the window are flagged, not forbidden. Major only until minor scale positions exist.
