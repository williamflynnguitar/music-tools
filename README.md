# Music tools

Small, self-contained browser tools for jazz guitar practice and teaching, by
William Flynn. Each tool is a single HTML file with no setup required, made
to open on a phone in the practice room.

**Live:** https://williamflynnguitar.github.io/music-tools/

From the handbook:

- **fretboard/** — movable scale fingerings, eleven scales, every key
- **scales-deck/** — Scale Practice 101 as a step-through deck of cards
- **arpeggios-deck/** — arpeggio practice: parallel shapes, HDA and VDA
- **shell-builder/** — build shell voicings by rule, then comp them through
  the changes
- **inversion-drill/** — drop-2 and drop-3 inversions up the neck
- **voice-leading/** — resolve to the nearest inversion instead of jumping
- **quartal-voicings/** — fourths on every degree, planing, two tunes
- **triad-voicings/** — closed and open triads on every string set
- **comping-rhythms/** — Charleston: swing comping rhythms in five families

Stand-alone:

- **line-ladder/** — an improvisation line generator for any instrument
- **two-and-four/** — a time trainer that clicks on beats 2 and 4, then fades
  out so your inner clock does the work
- **chartwright/** — chord-chart editor with self-adjusting spacing; charts
  save on your device
- **composition-assignments/** — prompt cards and seed-material tools for
  composition students
- **stageplot/** — stage plots for the jazz ensembles at Somewhere Works:
  layout, monitors, inputs and house gear on a page for the sound tech
- **box-buddy/** — chord-box handouts for band directors

These tools are the companion to the **Jazz Guitar Technique Handbook, 3rd
edition**; the fingering and exercise data is derived from it. The book itself
is not included here.

## License

[CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/) — free to
share and adapt for non-commercial use, with attribution. See [LICENSE](LICENSE).
