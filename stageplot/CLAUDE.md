...
v1 hardcoded the fall 2026 WSU rosters into the app; v2 moved them to
`samples/` as files anyone can load and edit.
...
The Examples row fetches `samples/*.json`. That works on GitHub Pages and
fails on `file://`, where browsers block fetching sibling files — the app says
so and points at Load. Not worth embedding the files in the page: they carry
student names, and keeping names out of `index.html` is the point of v2
(`check.js` greps for every name in the sample files and fails if one appears).
...
`node check.js --samples` rewrites `samples/*.json` by migrating `samples/v1/`.

Samples are the fall 2026 WSU ensembles as saved plots — Group A/B/C, Combo
A/B/C, Big Band — examples to load and edit, not app structure.
