...
**Group A** — Liz Graber (keys/vocals), Alex Kenney (guitar), Sean McDermott (bass), Caden Kennedy (drums)

**Group B** — Anna Luttrell (voice), James Barton (sax), Evan Shaw (sax), Sid Linafelt (guitar), Liz Graber (keys/voice), Kyler Heckard (bass), Caden Kennedy (drums), A.J. Banks (drums)
- Two drummers, one kit. Mark both as sharing the `kit` chair.

**Group C** — Emmalynn Reilly, Issy Lynn, Liam Wallace, Jack Wilson (vocals); Sagan Plantz (guitar); Michael Boyle (bass); A.J. Banks, C.J. Strecker (drums)
- Four vocalists, two drummers sharing one kit. This preset should show a wedge warning immediately (see §7).

**Fusion Combo** — guitar, sax, sax, bass, drums (no names yet; leave name fields blank)

**Combo A** — James Hoerst (tenor sax), James Barton (tenor sax), Dawson McNeal (trumpet), Evan Swope (trombone), Dexter Curry (piano → house keyboard), Julian Urrego (guitar), Ian Knickrehm (bass), Brendan Thompson (drums)

**Combo B** — Isaiah Epp (alto sax/flute), Evan Shaw (bari sax), Parker Holuska (trumpet), Aaron Buller (trumpet), Zach Rodenbaugh (piano → house keyboard), Phoenix Kirk (bass), Elijah Gegen (drums)

**Combo C** — Noah Georgas (alto sax), Hunter Bocan (trumpet), Alton Porter (trumpet), Victoria McClure (trombone), Andy Bunck (trombone), Michael Castillo (bass trombone), Sid Linafelt (guitar), Isaac Waterman (bass)
- No drums, no piano. Six horns. Default layout: horns in a shallow arc downstage, guitar and bass upstage. Do not place the house kit.

**Big Band 1 / Big Band 2** — 5 saxes, 8 brass (4 trumpets, 4 trombones), guitar, piano (house keyboard), bass, drums. Names blank. Default layout: sax row downstage, trombones behind, trumpets behind that (on riser if `riser.count > 0`, else standing), rhythm section stage left. 17 people on a 24×12 deck is tight; the preset should render and let the director decide.
...
Shared personnel across ensembles (Liz Graber in A and B, Caden Kennedy in A and B, A.J. Banks in B and C, Sid Linafelt in Group B and Combo C, James Barton in Group B and Combo A, Evan Shaw in Group B and Combo B) is expected; do not deduplicate.
...
- Two people assigned the same chair (two drummers, one kit) → render both names on the kit, print "Caden Kennedy / A.J. Banks (share kit)" and one set of drum inputs, not two.
...
- [ ] Changeover sheet between Group A and Group B lists Liz Graber and Caden Kennedy as staying, the kit as staying, and two saxes plus a second drummer as arriving.
